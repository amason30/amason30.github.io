...
tribble(
  ...
  linebreak("Soil sampling/Field research -- Amplicon Sequencing -- Data Analysis/Visualization Soil microbiome/necrobiome -- Metatranscriptomics -- Biostatistics -- Reproducible Research -- Machine learning"),
...
pubs <- bib_entries %>%
  mutate(authors = format_md2tex(authors),
         ...
         doi = if_else(is.na(doi), "", paste0("\\href{https://doi.org/",doi,"}{",doi,"}"))) %>%
  mutate(tex = paste0(authors," (",year,"). ",title," ",journal,". doi: ",doi)) %>%
  pull(tex) %>%
